...
function toast(text, persistent=false) {
  ...
  if (!persistent) setTimeout(() => capabilityTip.classList.remove("show"), 2600);
}
...
async function loadSettings() {
  ...
  try {
    ...
    s = { api_key: s.api_key||r.api_key||'', base_url: s.base_url||r.base_url||'https://api.deepseek.com/v1', model: s.model||r.model||'deepseek-chat', mock_mode: localStorage.getItem("lifepath_mock_mode")!==null?(localStorage.getItem("lifepath_mock_mode")==='true'):!!r.mock_mode };
  } catch(e){}
  ...
  if(mockToggleDesc) mockToggleDesc.textContent = `当前：${state.mock?"开启":"关闭"}`;
}
...
hiddenFileInput.onchange = async function() {
  ...
  const loading = $("#uploadLoading"); loading.style.display='flex';
  const fd = new FormData();
  for(const f of this.files) fd.append('files', f);
  const s = getStoredSettings();
  fd.append('api_key', s.api_key); fd.append('base_url', s.base_url); fd.append('model', s.model); fd.append('mock', String(s.mock_mode));
  try {
    await api('/api/upload-profile-files', {method:'POST', body: fd});
    toast("资料分析完成，画像已更新！");
    loadFileList(); loadMemoryData();
  } catch(e) { toast("上传失败: "+e.message, true); }
  finally { loading.style.display='none'; this.value=''; }
};
...
function renderMessages(messages) {
  ...
  messages.forEach(msg => {
    ...
    row.innerHTML=`<div class="avatar">${msg.role==="assistant"?"AI":"你"}</div><div class="bubble">${msg.content}</div>`;
...
async function sendMessage() {
  ...
  aiRow.innerHTML=`<div class="avatar">AI</div><div class="bubble"></div>`;
  ...
  try {
    const response = await fetch('/api/chat/send',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({conversation_id:state.conversationId,message:text,api_key:settings.api_key,base_url:settings.base_url,model:settings.model,mock:state.mock,main_mode:mapMode(state.primaryMode),sub_mode:mapSub(state.capability)})});
    ...
    while(true) {
      ...
      for(const line of chunk.split('\n').filter(l=>l.trim())) {
        try {
          ...
          if(data.type==='text') { fullReply+=data.content; aiBubble.textContent=fullReply; messageList.scrollTop=messageList.scrollHeight; }
